...
- Starlette has five published 2026 advisories whose stated fixed releases (1.0.1–1.3.1) are not available/compatible with the current FastAPI release. CI ignores only those documented IDs and blocks every other advisory; remove the exception immediately when a compatible fixed release is published.
